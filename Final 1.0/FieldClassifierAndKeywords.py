...
segment('载入分词程序')
os.system('clear')
def FieldClassifierAndKeywords(question):
##读入问题,调用分词工具分词,同时去除标点符号
    delset = string.punctuation
    question = question.translate(None, delset)
    questionTag = typeClassify(question)
    f = open("input.txt","w")
    ...
    p = subprocess.Popen(command, stdout=pos_file, shell=False,stderr=devnull)
    p.wait()
##s就是pos后的question
    pos_file.close()
    devnull.close()
...
def typeClassify(question):
    # Use regex to classify
    result = regexClassify(question)
...
def nbClassifier(question):
    ...
    #fout.write(s)
    #fout.close()
    tag=tagQues(question,wordSet)
    return tag



def regexClassify(question):
...
def target(question):
    if pattern_person.search(question.decode('utf8')) is not None\
        or pattern_loc.search(question.decode('utf8')) is not None:
        return "name"
    elif pattern_integer.search(question.decode('utf8')) is not None \
            or pattern_decimal.search(question.decode('utf8')) is not None:
        return "quantity"
    elif pattern_time.search(question.decode('utf8')) is not None:
        return "time"
    else:
