...
class answer2:
	...
	def select_per(self, words, tags,  a, b, weight):
		...
		for i in range(a, b):
			if tags[i]=='NR':
				temp.append(words[i])
		for x in xrange(0,weight*weight):
			self.candidates_per+=temp
			...
			for arti in xrange(num_of_article+1, len(arr)):
				...
				for sen in xrange(len(arr[arti])):
					...
					for j in range(len(punc)-1):
						...
						self.select_per(words, tags, punc[j], punc[j+1], len(key_match_set_subSentence))
								#print ('')
					#except ValueError:
					#    print ('!!!!!!!!!!!!!!')
					...
					for num in range(len(words)):
						if words[num] in info:
							self.select_per(words, tags, 0, len(words), len(key_match_set_sentence))
