...
class Handler(BaseHandler):
    ...
    client = MongoClient()
    db = client.proj
    collection = db.news_fenghuang_militarty

        
    @every(minutes=24 * 60)
    def on_start(self):
        self.crawl('http://news.ifeng.com/listpage/4550/20160324/1/rtlist.shtml', callback=self.index_page)

    @config(age=24 * 60 * 60)
    def index_page(self, response):
        
        for each in response.doc('.newsList a[href^="http"]').items():
            if re.match("http:\/\/news.ifeng.com\/a\/\d+\/\S+.shtml", each.attr.href) or re.match("http:\/\/news.ifeng.com\/mil\/\d+\/\S+.shtml", each.attr.href):
                self.crawl(each.attr.href, priority=9, callback=self.detail_page)
        arr= [x.attr.href for x in response.doc('.r_end a, .m_page a').items()]
        self.crawl(arr, callback=self.index_page)
