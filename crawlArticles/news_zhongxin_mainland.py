...
class Handler(BaseHandler):
    ...
    client = MongoClient()
    db = client.proj
    collection = db.news_zhongxin_mainland
 
    url1 = "http://www.chinanews.com/scroll-news/gn/"
    url2 = "/news.shtml"
    

    @every(minutes=24 * 60)
    def on_start(self):
        self.crawl('http://www.chinanews.com/scroll-news/gn/2016/0331/news.shtml', callback=self.index_page)

    @config(age=24 * 60 * 60)
    def index_page(self, response):
        
        for each in response.doc('.content_list a[href^="http"]').items():
            if re.match("http://www.chinanews.com\/gn\/\d{4}\/\d{2}\-\d{2}\S+.shtml", each.attr.href):
