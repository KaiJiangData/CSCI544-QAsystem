...
class Handler(BaseHandler):
    ...
    client = MongoClient()
    db = client.proj
    collection = db.news_zhongxin_military
 
    url1 = "http://www.chinanews.com/scroll-news/mil/"
    url2 = "/news.shtml"
    

    @every(minutes=24 * 60)
    def on_start(self):
        self.crawl('http://www.chinanews.com/scroll-news/mil/2016/0330/news.shtml', callback=self.index_page)

    @config(age=24 * 60 * 60)
    def index_page(self, response):
        
        for each in response.doc('.content_list a[href^="http"]').items():
            if re.match("http://www.chinanews.com\/mil\/\d{4}\/\d{2}\-\d{2}\S+.shtml", each.attr.href):
                self.crawl(each.attr.href, priority=9, callback=self.detail_page)
        
        url = response.url
        newdd = int(url[48:50])-1
        newmm = int(url[46:48])
        newyy = int(url[41:45])
        if(newdd==0):
            newmm-=1
